...
| phase (if not `const fn`) | runtime   | compile time | compile time | run time    |
...
The 4 modes above are possibly enough for a language without subtyping, or maybe be there could be 2 variants of pi mode, one's argument can be inferred whereas the other's cannot.
...
If the argument of the dependent mode is being dependent on another argument, then the mode cannot be initialized as normal mode.
...
| phase (if not `const fn`) | runtime   | compile time | compile time | run time    | depends     |
